...
func (a *addrs) Get(hash *common.Uint168) (*wutil.Addr, error) {
	a.RLock()
	defer a.RUnlock()
	...
	return wutil.NewAddr(hash, script, addrType), nil
}

// get all Addrs from database
func (a *addrs) GetAll() ([]*wutil.Addr, error) {
	a.RLock()
	defer a.RUnlock()

	var addrs []*wutil.Addr
	...
	for rows.Next() {
		...
		addrs = append(addrs, wutil.NewAddr(hash, script, addrType))
	}
