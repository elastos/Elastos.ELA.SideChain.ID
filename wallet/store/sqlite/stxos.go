...
func (s *stxos) Put(stxo *wutil.STXO) error {
	s.Lock()
	defer s.Unlock()
...
func (s *stxos) Get(op *util.OutPoint) (*wutil.STXO, error) {
	s.RLock()
	defer s.RUnlock()
	...
	var utxo = wutil.UTXO{Op: op, Value: *value, LockTime: lockTime, AtHeight: atHeight, Address: *address}
	spendHash, err := common.Uint256FromBytes(spendHashBytes)
	if err != nil {
		return nil, err
	}

	return &wutil.STXO{UTXO: utxo, SpendTxId: *spendHash, SpendHeight: spendHeight}, nil
}

// get stxos of the given script hash from database
func (s *stxos) GetAddrAll(hash *common.Uint168) ([]*wutil.STXO, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *stxos) GetAll() ([]*wutil.STXO, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *stxos) getSTXOs(rows *sql.Rows) ([]*wutil.STXO, error) {
	var stxos []*wutil.STXO
	for rows.Next() {
		var opBytes []byte
		var valueBytes []byte
		var lockTime uint32
		...
		var utxo = wutil.UTXO{Op: outPoint, Value: *value, LockTime: lockTime,
		...
		stxos = append(stxos, &wutil.STXO{UTXO: utxo, SpendTxId: *spendHash,
