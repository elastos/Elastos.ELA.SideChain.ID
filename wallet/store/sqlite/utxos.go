...
func (u *utxos) Put(utxo *wutil.UTXO) error {
	u.Lock()
	defer u.Unlock()
...
func (u *utxos) Get(op *util.OutPoint) (*wutil.UTXO, error) {
	u.RLock()
	defer u.RUnlock()
	...
	return &wutil.UTXO{Op: op, Value: *value, LockTime: lockTime, AtHeight: atHeight, Address: *address}, nil
}

// get utxos of the given script hash from database
func (u *utxos) GetAddrAll(hash *common.Uint168) ([]*wutil.UTXO, error) {
	u.RLock()
	defer u.RUnlock()
...
func (u *utxos) getUTXOs(rows *sql.Rows) ([]*wutil.UTXO, error) {
	var utxos []*wutil.UTXO
	for rows.Next() {
		var opBytes []byte
		var valueBytes []byte
		var lockTime uint32
		...
		utxo := &wutil.UTXO{Op: outPoint, Value: *value, LockTime: lockTime,
