...
type Addrs interface {
	...
	Get(hash *common.Uint168) (*wutil.Addr, error)

	// get all addresss from database
	GetAll() ([]*wutil.Addr, error)
...
type UTXOs interface {
	...
	Put(utxo *wutil.UTXO) error

	// get a utxo from database
	Get(op *util.OutPoint) (*wutil.UTXO, error)

	// get utxos of the given address hash from database
	GetAddrAll(hash *common.Uint168) ([]*wutil.UTXO, error)

	// Get all UTXOs in database
	GetAll() ([]*wutil.UTXO, error)
...
type STXOs interface {
	...
	Put(stxo *wutil.STXO) error

	// get a stxo from database
	Get(op *util.OutPoint) (*wutil.STXO, error)

	// get stxos of the given address hash from database
	GetAddrAll(hash *common.Uint168) ([]*wutil.STXO, error)

	// Get all STXOs in database
	GetAll() ([]*wutil.STXO, error)
