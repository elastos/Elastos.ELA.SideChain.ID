...
func listBalanceInfo(wallet *wcli.Wallet) error {
	addrs, err := wallet.GetAddrs()
	if err != nil {
		return errors.New("get wallet addresses failed")
	}

	return wcli.ShowAccounts(addrs, nil, wallet)
}

func newSubAccount(password []byte, wallet *wcli.Wallet) error {
	var err error
	...
	return wcli.ShowAccounts(addrs, programHash, wallet)
}

func addMultiSignAccount(context *cli.Context, wallet *wcli.Wallet, content string) error {
...
func accountAction(context *cli.Context) {
	...
	if context.Bool("list") {
		if err := wcli.ShowAccountInfo([]byte(pass)); err != nil {
