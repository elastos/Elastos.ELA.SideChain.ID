...
func CreateTransaction(c *cli.Context, wallet *wcli.Wallet) error {
...
func createTransaction(c *cli.Context, wallet *wcli.Wallet) (*core.Transaction, error) {
	feeStr := c.String("fee")
	if feeStr == "" {
	...
	from := c.String("from")
	if from == "" {
		from, err = wcli.SelectAccount(wallet)
		if err != nil {
			return nil, err
		}
	}
...
func createMultiOutputTransaction(c *cli.Context, wallet *wcli.Wallet, path, from string, fee *common.Fixed64) (*core.Transaction, error) {
	...
	for scanner.Scan() {
		...
		multiOutput = append(multiOutput, &wcli.Transfer{address, amount})
	}
...
func SignTransaction(password []byte, context *cli.Context, wallet *wcli.Wallet) error {
...
func signTransaction(password []byte, wallet *wcli.Wallet, tx *core.Transaction) (*core.Transaction, error) {
	...
	password, err = wcli.GetPassword(password, false)
	if err != nil {
		return nil, err
	}
...
func SendTransaction(password []byte, context *cli.Context, wallet *wcli.Wallet) error {
