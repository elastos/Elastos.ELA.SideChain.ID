...
import (
	...
	"github.com/elastos/Elastos.ELA.SideChain.ID/wallet/wcli/database"
...
func Create(password []byte) error {
	...
	return db.AddAddress(mainAccount.ProgramHash(),
		mainAccount.RedeemScript(), wutil.TypeMaster)
}
...
func (wallet *Wallet) NewSubAccount(password []byte) (*common.Uint168, error) {
	...
	err = wallet.AddAddress(account.ProgramHash(), account.RedeemScript(), wutil.TypeSub)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, errors.New("[Wallet], Can not parse program hash to address")
	}

	// Notify SPV service to reload bloom filter with the new address
	_, err = jsonrpc.CallArray(jsonRpcUrl, "notifynewaddress", address)

	return account.ProgramHash(), err
}
...
func (wallet *Wallet) AddMultiSignAccount(M uint, publicKeys ...*crypto.PublicKey) (*common.Uint168, error) {
	...
	err = wallet.AddAddress(programHash, redeemScript, wutil.TypeMulti)
	if err != nil {
		return nil, err
	}
...
func (wallet *Wallet) createTransaction(fromAddress string, fee *common.Fixed64, lockedUntil uint32,
	...
	availableUTXOs = wutil.SortByValue(availableUTXOs) // Sort available UTXOs by value ASC
...
func (wallet *Wallet) removeLockedUTXOs(utxos []*wutil.UTXO) []*wutil.UTXO {
	var availableUTXOs []*wutil.UTXO
...
func InputFromUTXO(utxo *wutil.UTXO) *core.Input {
